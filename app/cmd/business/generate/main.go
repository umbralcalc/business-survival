// generate emits the business-survival widget shell (widget.html,
// test.html, build.sh) into app/business/. Re-run whenever the
// dashboard.Config in pkg/businessdash changes shape (controls,
// partitions, visualisation).
//
//	cd app && go run ./cmd/business/generate
//
...
//     a status message is posted once the SimMonths horizon is reached
...
//     so a portfolio/scenario change restarts the simulation
package main
...
// the slider readout text. Kept in sync with businessdash.actionColorHex.
const actionColor = "#b0447a"

// portfolioChoices lists the radio buttons that replace the portfolio
// slider. Indices match the businessdash.Portfolio* constants.
var portfolioChoices = []struct {
	Value int
	Label string
}{
	{businessdash.PortfolioBaseline, businessdash.PortfolioFullLabels[businessdash.PortfolioBaseline]},
	{businessdash.PortfolioRatesRelief, businessdash.PortfolioFullLabels[businessdash.PortfolioRatesRelief]},
	{businessdash.PortfolioStartup, businessdash.PortfolioFullLabels[businessdash.PortfolioStartup]},
	{businessdash.PortfolioIncubator, businessdash.PortfolioFullLabels[businessdash.PortfolioIncubator]},
	{businessdash.PortfolioMentoring, businessdash.PortfolioFullLabels[businessdash.PortfolioMentoring]},
	{businessdash.PortfolioBlend, businessdash.PortfolioFullLabels[businessdash.PortfolioBlend]},
}

// scenarioChoices lists the radio buttons that replace the scenario
// slider. Indices match the businessdash.Scenario* constants.
var scenarioChoices = []struct {
	Value int
	Label string
}{
	{businessdash.ScenarioBaseline, businessdash.ScenarioFullLabels[businessdash.ScenarioBaseline]},
	{businessdash.ScenarioRecession, businessdash.ScenarioFullLabels[businessdash.ScenarioRecession]},
	{businessdash.ScenarioExpansion, businessdash.ScenarioFullLabels[businessdash.ScenarioExpansion]},
}
...
func postProcess(path string) error {
	...
	for _, step := range []func(string, string) (string, error){
		recolorControls,
		injectScopedStyles,
		replaceSliderWithRadios("portfolio", "business-portfolio", "Support portfolio", portfolioChoiceLabels()),
		replaceSliderWithRadios("scenario", "business-scenario", "Macro scenario", scenarioChoiceLabels()),
...
// and readout for the action-colour magenta.
...
const scopedStylesTemplate = `#{{.WidgetID}} .business-selector { display: flex; flex-direction: column; gap: 0.4em; font-size: 1rem; margin-bottom: 0.6em; }` +
	`#{{.WidgetID}} .business-selector-label { color: #2c3e50; font-weight: 600; }` +
	`#{{.WidgetID}} .business-options { display: flex; flex-direction: column; gap: 0.3em; }` +
	`#{{.WidgetID}} .business-options label { display: flex; align-items: center; gap: 0.4em; color: #2c3e50; cursor: pointer; }` +
	`#{{.WidgetID}} .business-options input[type="radio"] { accent-color: ` + actionColor + `; }`
...
// radio-button group. Same pattern as flood/energy widgets — the
// slider input itself is kept (display:none) so the existing
...
func replaceSliderWithRadios(sliderName, radioName, heading string, choices []choiceLabel) func(string, string) (string, error) {
	return func(html, _ string) (string, error) {
		...
		b.WriteString(`<div class="business-selector">`)
		fmt.Fprintf(&b, `<span class="business-selector-label">%s</span>`, heading)
		b.WriteString(`<div class="business-options">`)
...
// a partition state arrives with cumulativeTimesteps >= SimMonths, the
// worker is terminated and a status message is posted. Without this
// the inline driver ticks forever even after the custom iterations
// freeze their state.
func injectTerminationHalt(html, _ string) (string, error) {
	...
	newTail := fmt.Sprintf(`if (el) el.textContent = applyReadout(r.template, r.decimals, msg.data);
                ...
                if (worker && msg.data.timesteps >= %d) { worker.terminate(); worker = null; setStatus('10-year simulation complete. Use Reset to rerun.'); }
            } else if (msg.type === 'status') {`,
		businessdash.SimMonths)
...
// page re-publishes the current slider values once the driver reports
// that it's ready. Same rationale as the flood widget — the initial
// setActions message races the worker's driver-load, so the first
// step (or several) can pick up stale defaults on Reset.
...
// (portfolio and scenario groups) to their hidden sliders and clicks
// the Reset button on every change.
